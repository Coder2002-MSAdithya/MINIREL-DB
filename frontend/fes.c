...
void parser ()
{
	char buff[BUFSIZ + 2], params[MAXPARAS][PARALEN], *ptr[MAXPARAS];
	short loop, rcode;
...
{
	char tokenbuff[PARALEN], *buffstart;
	short loop, len, command, state, scount, pcount;
	int tcode;
	...
	if ((tcode >= CREATEDB) && (tcode <= DROPINDEX))
	{
		...
		do
		{
			...
			else if (table[command][state + 1].find == 0 && table[command][state + 1].tcode != 0)
			{
				state += 1;
			}
...
/* len: pointer to token length variable */
{
	...
	*len = 0;  /* initialize token length to zero  */
	...
	/* look at the first character */
		...
		*len += 1;
		...
		if (strcmp(btokenp, "createdb") == EQUAL)
			return (CREATEDB);
		if (strcmp(btokenp, "destroydb") == EQUAL)
			return (DESTROYDB);
		if (strcmp(btokenp, "opendb") == EQUAL)
			return (OPENDB);
		if (strcmp(btokenp, "closedb") == EQUAL)
			return (CLOSEDB);
		if (strcmp(btokenp, "create") == EQUAL)
			return (CREATE);
		if (strcmp(btokenp, "destroy") == EQUAL)
			return (DESTROY);
		if (strcmp(btokenp, "load") == EQUAL)
			return (LOAD);
		if (strcmp(btokenp, "print") == EQUAL)
			return (PRINT);
		if (strcmp(btokenp, "sort") == EQUAL)
			return (SORT);
		if (strcmp(btokenp, "quit") == EQUAL)
			return (QUIT);
		if (strcmp(btokenp, "select") == EQUAL)
			return (SELECT);
		if (strcmp(btokenp, "project") == EQUAL)
			return (PROJECT);
		if (strcmp(btokenp, "join") == EQUAL)
			return (JOIN);
		if (strcmp(btokenp, "insert") == EQUAL)
			return (INSERT);
		if (strcmp(btokenp, "delete") == EQUAL)
			return (DELETE);
		if (strcmp(btokenp, "intersect") == EQUAL)
			return (INTERSECT);
		if (strcmp(btokenp, "union") == EQUAL)
			return (UNIONOP);
		if (strcmp(btokenp, "on") == EQUAL)
			return (ON);
		if (strcmp(btokenp, "key") == EQUAL)
			return (KEY);
		if (strcmp(btokenp, "size") == EQUAL)
			return (SIZE);
		if (strcmp(btokenp, "is") == EQUAL)
			return (IS);
		if (strcmp(btokenp, "from") == EQUAL)
			return (FROM);
		if (strcmp(btokenp, "into") == EQUAL)
			return (INTO);
		if (strcmp(btokenp, "to") == EQUAL)
			return (TO);
		if (strcmp(btokenp, "where") == EQUAL)
			return (WHERE);
		if (strcmp(btokenp, "and") == EQUAL)
			return (AND);
		if (strcmp(btokenp, "help") == EQUAL)
			return (HELP);
		if (strcmp(btokenp, "buildindex") == EQUAL)
			return (BUILDINDEX);
		if (strcmp(btokenp, "dropindex") == EQUAL)
			return (DROPINDEX);
		if (strcmp(btokenp, "for") == EQUAL)
			return (FOR);
		if (strcmp(btokenp, "") == EQUAL)
			return (ILLEGAL);
